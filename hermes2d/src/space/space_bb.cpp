...
namespace Hermes
{
  namespace Hermes2D
  {
    ...
    void SpaceBB<Scalar>::init(Shapeset* shapeset, int p_init)
    {
      ...
      if(p_init < 1) 
        throw Hermes::Exceptions::Exception("P_INIT must be >=  1 in an Bernstein Bezier space.");
 ...
 template<typename Scalar>
    ...
    template<typename Scalar>
    void SpaceBB<Scalar>::update_constrained_nodes(Element* e, EdgeInfo* ei0, EdgeInfo* ei1, EdgeInfo* ei2, EdgeInfo* ei3)
    {
      int j, k;
