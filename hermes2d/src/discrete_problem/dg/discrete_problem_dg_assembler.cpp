      ...
      for(unsigned int i = 0; i < current_state->num; i++)
      {
        ...
        for(int j = i - 1; j >= 0; j--)
          ...
          if(!existing_ns)
          {
            ...
            if(current_neighbor_searches[i]->set_active_edge_multimesh(current_state->isurf) && (i >= this->spaces_size || spaces[i]->get_type() == HERMES_L2_SPACE || spaces[i]->get_type() == HERMES_L2_SEMI_SPACE))
              DG_intra = true;
