		...
		for(int j = 0; j<ndof; j++){ //Spalten durchlaufen
				for(int indx = Ap_mass[j]; indx<Ap_mass[j+1];indx++){	
							...
							if(((mass=Ax_mass[indx])!=0.)&&(j<i)){
								...
								f = (mass/time_step+ (diff+mass)/2.)*(u_high[i]- u_high[j])
														-(mass/time_step - (diff+mass)/2.) *(u_old[i]- u_old[j]);	
										...
										f = C*lumped_matrix->get_Ax()[i]*(u_L[j]-u_L[i])/time_step; 
										if(f>Q_plus[i]) Q_plus[i] = f;				
										if(f<Q_minus[i]) Q_minus[i] = f;			
										f= C*lumped_matrix->get_Ax()[j]*(u_L[i]-u_L[j])/time_step;
