...
public:
//Constructor
  ConvectionOperator_x(int num_of_equations = 5):  WeakForm<double>(num_of_equations){
for(int i = 0; i<5; i++)
...
{
public:
  ConvectionOperator_y(int num_of_equations = 5):  WeakForm<double>(num_of_equations){
for(int i = 0; i<5; i++)
...
double a,b;
	double* Ax = matrix_K->get_Ax();
	int* Ai = matrix_K->get_Ai();
	int* Ap = matrix_K->get_Ap();

		for(int j = ndof_g; j<size; j++){ //Spalten durchlaufen
