...
public:
 	void project_lumped( SpaceSharedPtr<double> space, MeshFunctionSharedPtr<double>  source_meshfn,
                             double* target_vec, MatrixSolverType matrix_solver = SOLVER_UMFPACK,bool* fct =NULL, CSCMatrix<double>*  mat  = NULL);


	void project_lumped(Hermes::vector<SpaceSharedPtr<double> >  spaces, Hermes::vector<MeshFunctionSharedPtr<double> > source_meshfns,
          double* target_vec, Hermes::MatrixSolverType matrix_solver = SOLVER_UMFPACK,bool* fct =NULL,CSCMatrix<double>*  mat  = NULL);


protected:
 	void project_internal( SpaceSharedPtr<double> space, WeakForm<double>* wf, double* target_vec,
                               MatrixSolverType matrix_solver,bool* fct =NULL, CSCMatrix<double>*  mat = NULL);
