...
CustomWeakFormCNTG cntg(time_step);
...
//Lowview.show(u_prev_time);
	...
	CSCMatrix<double> * mat_cntg = new CSCMatrix<double> ; 
	double* u_H =NULL;	
...
int time_counter =1.;
			...
			DiscreteProblem<double> * dp_cntg = new DiscreteProblem<double> (&cntg, space);
			dp_mass->set_linear(true,false);
			dp_convection->set_linear(true,false);
			dp_surf->set_linear(true,false);
			dp_cntg->set_linear(true,false);
...
dp_cntg->assemble(mat_cntg);

			dp_mass->assemble(mass_matrix); 
			//mass_matrix->add_sparse_matrix(mat_cntg);	
			CSCMatrix<double> * lumped_matrix = NULL;
			dp_convection->assemble(conv_matrix);
			CSCMatrix<double> * diffusion = NULL;
		...
		if(FCT==true){
					...
					diffusion = artificialDiffusion(conv_matrix);
			...
			high_matrix->add_sparse_matrix(mat_cntg);
			...
			high_rhs->add_sparse_matrix(mat_cntg);		
...
antidiffusiveFlux(mass_matrix,mat_cntg,lumped_matrix,conv_matrix,diffusion,u_H, coeff_vec_2,coeff_vec, coeff_vec_3, P_plus, P_minus, Q_plus, Q_minus,Q_plus_old, Q_minus_old, R_plus, R_minus);
...
if(FCT==true)
{
delete lumped_matrix; delete diffusion;
}
