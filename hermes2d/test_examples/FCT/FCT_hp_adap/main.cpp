...
//Adaptivity loop
	...
	//-------------------------solution of lower order------------	
		...
		//-------------solution of higher order
			...
			//dp->assemble(high_matrix,vec_high_rhs);
		...
		//---------------------------------------antidiffusive fluxes-----------------------------------	
			 ...
			 antidiffusiveFlux(dof_list,dof_list_2,mass_matrix,lumped_matrix,conv_matrix,diffusion,u_H, u_L,coeff_vec, coeff_vec_3, 
									P_plus, P_minus, Q_plus, Q_minus, R_plus, R_minus);
...
if(ps==2) u_prev.copy(&u_new);
		...
		if(ps==1){
			...
			changed = h_p_adap(ref_space,&u_prev_time, &u_new,&R_h_1,&R_h_2,  &massmatrix,
			adapting,dof_list,dof_list_2, al, &p1_elements,&neighbor,h_min,h_max, ts,ps,smooth_elem);			
