	...
	if(al!=NULL){
			...
			for(unsigned int i = 0; i < al->get_cnt(); i ++){
			 	for(unsigned int j = (i+1); j < al->get_cnt(); j ++){
					...
					f = (mass_matrix->get(al->get_dof()[i],al->get_dof()[j])+ diffusion->get(al->get_dof()[i],al->get_dof()[j])/2.)*
																																			(u_high[al->get_dof()[i]]- u_high[al->get_dof()[j]])
									-(mass_matrix->get(al->get_dof()[i],al->get_dof()[j])- diffusion->get(al->get_dof()[i],al->get_dof()[j])/2.) *
		...
		//fuer list_2
			for(unsigned int i = 0; i < al->get_cnt(); i ++){
				...
				f = (mass_matrix->get(al->get_dof()[i],al_2->get_dof()[j])+ diffusion->get(al->get_dof()[i],al_2->get_dof()[j])/2.)*
																																			(u_high[al->get_dof()[i]]- u_high[al_2->get_dof()[j]])
									-(mass_matrix->get(al->get_dof()[i],al_2->get_dof()[j])- diffusion->get(al->get_dof()[i],al_2->get_dof()[j])/2.) *
...
void lumped_flux_limiter(AsmList<Scalar>* al,AsmList<Scalar>* al_2,UMFPackMatrix<Scalar>* mass_matrix,UMFPackMatrix<Scalar>* lumped_matrix, Scalar* u_L, Scalar* u_H, Scalar* P_plus, Scalar* P_minus, Scalar* Q_plus, Scalar* Q_minus, Scalar* R_plus, Scalar* R_minus, int* smooth_dof=NULL)
{
	...
	if(al!=NULL){
		...
		for(unsigned int i = 0; i < al->get_cnt(); i ++){
		 	for(unsigned int j = (i+1); j < al->get_cnt(); j ++){
				if(al->get_dof()[i]!=al->get_dof()[j]){		
					f = mass_matrix->get(al->get_dof()[i],al->get_dof()[j])*time_step*(u_H[al->get_dof()[i]]- u_H[al->get_dof()[j]]);								
	...
	for(unsigned int i = 0; i < al->get_cnt(); i ++){
		 	for(unsigned int j = 0; j < al_2->get_cnt(); j ++){
				if(al->get_dof()[i]!=al_2->get_dof()[j]){		
					f = mass_matrix->get(al->get_dof()[i],al_2->get_dof()[j])*time_step*(u_H[al->get_dof()[i]]- u_H[al_2->get_dof()[j]]);				
