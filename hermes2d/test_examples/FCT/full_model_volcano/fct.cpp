...
SimpleVector<double>*  vec_bdry,double*  vec_source,double* u_L, double* flux_scalar, double* P_plus, double* P_minus, double* Q_plus, double* Q_minus, double* R_plus, double* R_minus, int dof_rho, double time_step, double gamma )
{ 
...
int dof_v_y_start = dof_rho + dof_v_x ;
	...
	for(int i=0; i<ndof;i++){
						...
						dt_u_L[i] += (vec_bdry->get(i) + vec_source[i]);
						dt_u_L[i] *=1./lumped;	
	...
	f_rho = mass*(dt_u_L[i]- dt_u_L[j]) + diff*(u_L[i]- u_L[j]);
		...
		//Berechnung von P&Q für gas phase
		for(int j = ndof_g; j<(ndof_g+dof_rho); j++){ //Spalten durchlaufen
				for(int indx = Ap_mass[j]; indx<Ap_mass[j+1];indx++){	
					int i = Ai_mass[indx];	
...
//particle flux
	...
	//density- rho
			...
			if(alpha_v_x>alpha_rho) alpha = alpha_rho;
				else alpha = alpha_v_x;	
				
				if(alpha_v_y<alpha_rho) alpha = alpha_v_y;
