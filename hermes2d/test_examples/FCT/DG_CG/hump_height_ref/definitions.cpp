...
//---------------Weakform Convection-Diffusion-Reaction-----------
...
CustomWeakFormRHS::CustomWeakFormRHS(MeshFunctionSharedPtr<double> sln_exact,MeshFunctionSharedPtr<double> sln_prev_time, double theta) : WeakForm<double>(1)
{
...
double CustomWeakFormRHS::calculate_a_dot_v(double x, double y, double vx, double vy) const
{
 
 return  x*vx + y*vy;
}

Ord CustomWeakFormRHS::calculate_a_dot_v(Ord x, Ord y, Ord vx, Ord vy) const
{
  return Ord(10);
}

double CustomWeakFormRHS::upwind_flux(double u_cent, double u_neib, double a_dot_n) const
{
  return a_dot_n * (a_dot_n >= 0 ? u_cent : u_neib);
}

Ord CustomWeakFormRHS::upwind_flux(Ord u_cent, Ord u_neib, Ord a_dot_n) const
{
  return a_dot_n * (u_cent + u_neib);
}
...
//------------DG Vector Form--------------
     double CustomWeakFormRHS::CustomVectorFormInterface::value(int n, double *wt, DiscontinuousFunc<double> **u_ext, Func<double> *v,
...
double v_x =2.; double v_y =3.; 
   ...
   double a_dot_n = static_cast<CustomWeakFormRHS*>(wf)->calculate_a_dot_v(v_x, v_y, e->nx[i], e->ny[i]);
...
double mid_v= v->val[i];
      ...
      result += wt[i] * static_cast<CustomWeakFormRHS*>(wf)->upwind_flux(sln_prev_time->val[i], sln_prev_time->val_neighbor[i], a_dot_n) * jump_v;	
  ...
  return (-result*(1.-theta));
    ...
    Ord CustomWeakFormRHS::CustomVectorFormInterface::ord(int n, double *wt, DiscontinuousFunc<Ord> **u_ext, Func<Ord> *v, Geom<Ord> *e,
...
Ord result = Ord(10); 
  return result;
      ...
      VectorFormDG<double>* CustomWeakFormRHS::CustomVectorFormInterface::clone() const
{
  return new CustomWeakFormRHS::CustomVectorFormInterface(*this);
}


//----------Dirichlet Vector surface Form
double CustomWeakFormRHS::CustomVectorFormSurface::value(int n, double *wt, Func<double> *u_ext[], Func<double> *v,
                                                Geom<double> *e, Func<double> **ext) const
{
  double result = 0;
 Func<double>* exact = ext[0];
   ...
   for (int i = 0; i < n; i++)
	{ 				
			...
			double a_dot_n = static_cast<CustomWeakFormRHS*>(wf)->calculate_a_dot_v(v_x, v_y, e->nx[i], e->ny[i]);
			double grad_dot_n = static_cast<CustomWeakFormRHS*>(wf)->calculate_a_dot_v(exact->dx[i], exact->dy[i], e->nx[i], e->ny[i]);

			result -= wt[i]*(static_cast<CustomWeakFormRHS*>(wf)->upwind_flux(0., exact->val[i], a_dot_n))*v->val[i];
...
Ord CustomWeakFormRHS::CustomVectorFormSurface::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *v, Geom<Ord> *e, Func<Ord> **ext) const
{
  Ord result = Ord(10);
  return result;
}

VectorFormSurf<double>* CustomWeakFormRHS::CustomVectorFormSurface::clone() const
{
  return new CustomWeakFormRHS::CustomVectorFormSurface(*this);
}
