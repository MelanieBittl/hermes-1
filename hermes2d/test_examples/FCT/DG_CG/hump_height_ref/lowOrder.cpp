...
void Low_Order::assemble_Low_Order(CSCMatrix<double> * conv_matrix,CSCMatrix<double>* diffusion, CSCMatrix<double> * lumped_matrix,CSCMatrix<double> * dg_surface_matrix,CSCMatrix<double> * reac_matrix_lumped)
{
	...
	low_matrix->create(dg_surface_matrix->get_size(),dg_surface_matrix->get_nnz(), dg_surface_matrix->get_Ap(), dg_surface_matrix->get_Ai(),dg_surface_matrix->get_Ax());	
				low_matrix->multiply_with_Scalar(-1.); //damit surface richtiges Vorzeichen!!
			low_matrix->add_sparse_matrix(diffusion); 
			low_matrix->add_sparse_matrix(conv_matrix);			
			low_matrix->add_sparse_matrix(reac_matrix_lumped);	
			lowmat_rhs->create(conv_matrix->get_size(),conv_matrix->get_nnz(), conv_matrix->get_Ap(),conv_matrix->get_Ai(),conv_matrix->get_Ax());
			lowmat_rhs->add_sparse_matrix(diffusion);
			lowmat_rhs->add_sparse_matrix(reac_matrix_lumped);	
...
//lumped_matrix = M_L !!!!!
//-----------------------------------------------------------------
double* Low_Order::solve_Low_Order(CSCMatrix<double> * lumped_matrix, double* u_n, double time_step,SimpleVector<double> * rhs_f  )
{
			...
			for(int i =0;i<ndof;i++) rhs[i]+=rhs_f->get(i);
