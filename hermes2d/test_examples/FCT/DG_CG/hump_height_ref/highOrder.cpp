...
void High_Order::assemble_High_Order(CSCMatrix<double> * conv_matrix, CSCMatrix<double> * mass_matrix,CSCMatrix<double> * dg_surface_matrix,CSCMatrix<double> * reac_matrix)
{
	if(high_matrix!=NULL) 
		high_matrix->free();	 		
	if(highmat_rhs!=NULL) 
		highmat_rhs->free();
			...
			high_matrix->add_sparse_matrix(reac_matrix);
			...
			highmat_rhs->add_sparse_matrix(reac_matrix);
...
//-----------------------------------------------------------------------------------------------
double* High_Order::solve_High_Order(double* u_n,SimpleVector<double> * rhs_f)
{
  ...
  vec_rhs->add_vector(coeff_vec_2); vec_rhs->add_vector(rhs_f);
