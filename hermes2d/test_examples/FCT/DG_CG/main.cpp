...
lumped_double[i] =0.0; coeff_vec[i]=0.0;
}*/
		if(as==1)
		{	
		//----------------------MassLumping M_L/tau--------------------------------------------------------------------
	...
	/*		DiscreteProblem<double> * dp_mass_2 = new DiscreteProblem<double> (&massmatrix, ref_space_h1);
			UMFPackMatrix<double> * mass_matrix_2 = new UMFPackMatrix<double> ;
			dp_mass_2->assemble(mass_matrix_2);
			UMFPackMatrix<double> * lumped_matrix_2 = massLumping(mass_matrix_2);
			lumped_matrix_2->multiply_with_Scalar(time_step);*/
...
//ref_ndof = ref_space_2->get_num_dofs(); 
	/*	DiscreteProblem<double> * dp_all = NULL;
		if(ts ==1)
			dp_all = new DiscreteProblem<double> (&wf_all, ref_space);
		else
			dp_all = new DiscreteProblem<double> (&wf_all_2, ref_space);
 ...
 dp_all->assemble(matrix, rhs); 
     if(solver->solve()) Solution<double>::vector_to_solution(solver->get_sln_vector(), ref_space, &high_sln);
		else throw Hermes::Exceptions::Exception("Matrix solver failed.\n");*/
