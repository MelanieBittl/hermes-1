...
namespace Hermes
{
	namespace Hermes2D
	{
		...
		///L2_SEMI_CG_SPACE represents a space of continuous p1-functions with discontinuous basisfunctions for higher orders..
		template<typename Scalar>
		class HERMES_API L2_SEMI_CG_Space : public Space<Scalar>
		{
		public:
			L2_SEMI_CG_Space();
			L2_SEMI_CG_Space(const Mesh* mesh, EssentialBCs<Scalar>* boundary_conditions, int p_init = 1,
				Shapeset* shapeset = NULL);

			L2_SEMI_CG_Space(const Mesh* mesh, int p_init = 1,	Shapeset* shapeset = NULL);
