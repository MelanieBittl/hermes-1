...
{	 
	...
	{	
		...
		ogProjection.project_global(ref_space,&u_prev_time, coeff_vec,  HERMES_L2_NORM); 
			Solution<double>::vector_to_solution(coeff_vec, ref_space, &u_prev);
			delete [] coeff_vec;
		}    
			
			smoothness_indicator_2(ref_space,&u_prev,&R_h_1,&R_h_2, smooth_elem,smooth_dof,al,mass_matrix,true);
			changed = h_p_adap(ref_space,&u_prev,&R_h_1,&R_h_2,&massmatrix, adapting,al, h_min,h_max, ts,as,smooth_elem, h_start);	
...
//sprintf(title, "End: Time %3.2f, timestep=%i", current_time,ts);
  ...
  if(ts==3000)
  {
	lin.save_solution_vtk(&u_prev, "mid_hpadap.vtk", "solution", mode_3D);
	ord.save_orders_vtk(ref_space, "mid_order.vtk");
	}
...
while (current_time < T_FINAL);
	...
	//ord.save_mesh_vtk(ref_space, "end_mesh.vtk");
