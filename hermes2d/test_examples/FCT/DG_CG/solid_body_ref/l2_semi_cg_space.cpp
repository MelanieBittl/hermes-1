...
namespace Hermes
{
  namespace Hermes2D
  {
    ...
    template<typename Scalar>
    L2_SEMI_CG_Space<Scalar>::L2_SEMI_CG_Space(const Mesh* mesh, EssentialBCs<Scalar>* essential_bcs, int p_init, Shapeset* shapeset)
    ...
    template<typename Scalar>
    L2_SEMI_CG_Space<Scalar>::L2_SEMI_CG_Space(const Mesh* mesh, int p_init, Shapeset* shapeset)
        ...
        if(en->bnd)
          if(this->essential_bcs != NULL)
            if(this->essential_bcs->get_boundary_condition(this->mesh->boundary_markers_conversion.get_user_marker(en->marker).marker) != NULL)
            {
              ...
              this->bc_data.push_back(this->get_bc_projection(surf_pos, order));

              int i = surf_pos->surf_num, j = e->next_vert(i);
              this->ndata[e->vn[i]->id].vertex_bc_coef = this->get_bc_projection(surf_pos, order) + 0;
              this->ndata[e->vn[j]->id].vertex_bc_coef = this->get_bc_projection(surf_pos, order) + 1;
            }
     }
      else
      {
