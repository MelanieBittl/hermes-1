...
void smoothness_indicator(SpaceSharedPtr<double>  space,MeshFunctionSharedPtr<double> sln_fct,MeshFunctionSharedPtr<double> R_h_1_fct,MeshFunctionSharedPtr<double> R_h_2_fct, int* smooth_elem_patch, int* smooth_dof, AsmList<double>* al,UMFPackMatrix<double> * mass_matrix,bool proj)
{
...
void smoothness_indicator_2(SpaceSharedPtr<double>  space,MeshFunctionSharedPtr<double> sln_fct,MeshFunctionSharedPtr<double> R_h_1_fct,MeshFunctionSharedPtr<double> R_h_2_fct, int* smooth_elem_patch, int* smooth_dof, AsmList<double>* al,UMFPackMatrix<double> * mass_matrix,bool proj)
{
...
//---------------Elemente mit Vertex(dof) bestimmen
	...
	RegSolution lin_sln(space);
	RegSolution lin_sln_dx(space);
	RegSolution lin_sln_dy(space);
	lin_sln.linear_approx(u_c, R_h_1,R_h_2);	  
	lin_sln_dx.linear_approx_d(d_u_c_dx, R_h_1);
	lin_sln_dy.linear_approx_d(d_u_c_dy, R_h_2); 
	 
/* SlopeLimiterSolution diff_sln(&lin_sln, space);
  diff_sln.set_diff_linear();
 SlopeLimiterSolution diff_sln_dx(&lin_sln_dx, space);
  diff_sln_dx.set_diff_linear();   
    SlopeLimiterSolution diff_sln_dy(&lin_sln_dy, space);
  diff_sln_dy.set_diff_linear(); */
  
   SlopeLimiterSolution limited_sln(&lin_sln, space);
  	limited_sln.limit_solution_according_to_detector(true); 
 SlopeLimiterSolution limited_sln_dx(&lin_sln_dx, space);
  	limited_sln_dx.limit_solution_according_to_detector(true); 
    SlopeLimiterSolution limited_sln_dy(&lin_sln_dy, space);
  	limited_sln_dy.limit_solution_according_to_detector(true);
  	
  		double* alpha_sln = limited_sln.get_alpha_first_order();
  		double* alpha_dx = limited_sln_dx.get_alpha_first_order();
  		double* alpha_dy = limited_sln_dy.get_alpha_first_order();
  	
  /*	 	ScalarView s1view("reg_sol", new WinGeom(0, 500, 500, 400));
  		s1view.show(&lin_sln);
  	ScalarView k1view("diff", new WinGeom(0, 0, 500, 400));
  		k1view.show(&diff_sln);
  
  	ScalarView sview("reg_soldx", new WinGeom(200, 500, 500, 400));
  		sview.show(&lin_sln_dx);
  	ScalarView kview("diffdx", new WinGeom(200, 0, 500, 400));
  		kview.show(&diff_sln_dx);
  		
  		  	ScalarView s2view("reg_soldy", new WinGeom(500, 500, 500, 400));
  		s2view.show(&lin_sln_dy);
  	ScalarView k2view("diffdy", new WinGeom(500, 0, 500, 400));
  		k2view.show(&diff_sln_dy);
...
//------------------------------------------------
	...
	//VolumeAverage vol_av; double a,b, a_x, b_x, a_y, b_y;
 ...
 //	a = vol_av.calculate_norm(e, space, &diff_sln);
 // 	b = vol_av.calculate_norm(e, space, &limited_sln);
 //	a_x = vol_av.calculate_norm(e, space, &diff_sln_dx);
  //	b_x = vol_av.calculate_norm(e, space,&limited_sln_dx);	
  //	a_y = vol_av.calculate_norm(e, space, &diff_sln_dy);
  //	b_y = vol_av.calculate_norm(e, space,&limited_sln_dy);	
  	
 

  	smooth_elem_patch[e->id]=0; 
  			smooth_fct_in_elem =0; 
			smooth_dx_in_elem =0; 
			smooth_dy_in_elem =0;  		
 // 	if(b_x>EPS)
  	//{		
  		if(alpha_dx[e->id] > 0.9) 			
  			smooth_dx_in_elem =1; 
  		if(alpha_dy[e->id]  > 0.9)   
  			smooth_dy_in_elem =1; 
  		if(alpha_sln[e->id]  > 0.9)   
  			smooth_fct_in_elem =1;   
  //	}
/*   if(b_y>EPS)
   {
   	if(alpha_dy[e->id]  > 0.9)   
