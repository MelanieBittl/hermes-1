...
int RiemannInvariants::get_bdry_info_short(double rho, double rho_v_x, double rho_v_y, double rho_energy, double n_x, double n_y, double t_x, double t_y,
	...
	if( solid==true){
		...
		return 4;	
				...
				if((lambda_1<0)&&(lambda_2_3>=0)&&(lambda_4<0)) printf("case1");
				else	if((lambda_1>=0)&&(lambda_2_3<0)&&(lambda_4>=0)) printf("case2");
				else	if((lambda_1>=0)&&(lambda_2_3>=0)&&(lambda_4<0)) printf("case3");
				else printf("get_bdry_info: eigenvalues = %f, %f, %f", lambda_1, lambda_2_3, lambda_4);
	return 5;
	 }
}
/*
