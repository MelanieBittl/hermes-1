bool* get_fct_dofs(Hermes::vector<SpaceSharedPtr<double> > spaces)
{
...
void antidiffusiveFlux(CSCMatrix<double>* mass_matrix,CSCMatrix<double>* lumped_matrix,CSCMatrix<double>* diffusion,CSCMatrix<double>* matrix_L,	double* u_L, double* flux_scalar, double* P_plus, double* P_minus, double* Q_plus, double* Q_minus, double* R_plus, double* R_minus, int dof_rho, int dof_v_x, int dof_v_y, int dof_e, bool* fct=NULL )
{ 

	int ndof = diffusion->get_size();
	double* u_rhs = new double[ndof];
...
int dof_v_y_start = dof_rho + dof_v_x ;
	...
	matrix_L->multiply_with_vector(u_L, u_rhs);
	SimpleVector<double>* vec_rhs = new SimpleVector<double>(ndof);	
	vec_rhs->zero();
	vec_rhs->add_vector(u_rhs);
  ...
  catch(Hermes::Exceptions::Exception e)
  {
    e.print_msg();
		...
		double* dt_u_L = lowOrd->get_sln_vector();  
...
void lumped_flux_limiter(CSCMatrix<double>* mass_matrix,CSCMatrix<double>* lumped_matrix, double* u_L, double* u_H, double* P_plus, double* P_minus, double* Q_plus, double* Q_minus,double* R_plus, double* R_minus, int dof_rho, int dof_v_x, int dof_v_y, int dof_e, bool* fct =NULL )
...
//if((fct!=NULL)&&(fct[i]==false)) { R_plus[i]= 1.0;R_minus[i]= 1.0;}
