  ...
  {		
		 ...
		 if(((static_cast<EulerKS*>(wf))->mirror_condition==true)||(solid==false)){ 
			//	bdry =(static_cast<EulerKS*>(wf))->riemann_invariants->get_bdry_info(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i],e->tx[i],e->ty[i], ext[4]->val[i], ext[5]->val[i], ext[6]->val[i],ext[7]->val[i], ghost_state, solid);
				...
				if((e->y[i]<=0.)&&(e->x[i]<=1.)&&(e->x[i]>=-1.))	{	solid = true; bdry =0;constant= 0.5;}
				else if((e->y[i]==1.)||(e->x[i]==1.)){ bdry=1;constant =1.;}
				else if(e->x[i]==-1.){bdry =2;constant= 0.5;}
...
//-----------------------------------------------------------
	...
	Euler_wf::Euler_wf(double kappa,double time_step, double theta,MeshFunctionSharedPtr<double>  rho_ext, MeshFunctionSharedPtr<double>  v1_ext, MeshFunctionSharedPtr<double>  v2_ext, MeshFunctionSharedPtr<double>  energy_ext, MeshFunctionSharedPtr<double>  prev_density, MeshFunctionSharedPtr<double>  prev_density_vel_x,  MeshFunctionSharedPtr<double>  prev_density_vel_y, MeshFunctionSharedPtr<double>  prev_energy, bool mirror_condition,int num_of_equations): WeakForm<double>(num_of_equations), euler_fluxes(new EulerFluxes(kappa)),kappa(kappa), riemann_invariants(new RiemannInvariants(kappa)), mirror_condition(mirror_condition),
    prev_density(prev_density), prev_density_vel_x(prev_density_vel_x), prev_density_vel_y(prev_density_vel_y), prev_energy(prev_energy), rho_ext(rho_ext), v1_ext (v1_ext), v2_ext(v2_ext), energy_ext (energy_ext), time_step(time_step), theta(theta) 
	{

 		add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_rho(kappa,theta,0));
 		add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_rho(kappa,theta,1));
 		add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_rho(kappa,theta,2));
 		add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_rho(kappa,theta,3));

    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_x( kappa,theta,0));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_x( kappa,theta,1));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_x( kappa,theta,2));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_x( kappa,theta,3));

    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_y( kappa,theta,0));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_y( kappa,theta,1));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_y( kappa,theta,2));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_vel_y( kappa,theta,3));

    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_e( kappa,theta,0));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_e( kappa,theta,1));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_e( kappa,theta,2));
    add_matrix_form_surf(new Euler_wf::EulerBoundaryBilinearForm_e( kappa,theta,3));

    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensity(theta,0));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensity(theta,1));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensity(theta,2));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensity(theta,3));

   add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelX(kappa,theta,0));
   add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelX(kappa,theta,1));
   add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelX(kappa,theta,2));
   add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelX(kappa,theta,3));

    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelY(kappa,theta,0));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelY(kappa,theta,1));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelY(kappa,theta,2));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormDensityVelY(kappa,theta,3));

    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormEnergy(kappa,theta,0));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormEnergy(kappa,theta,1));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormEnergy(kappa,theta,2));
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormEnergy(kappa,theta,3));

    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormTime(0,time_step));  //density
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormTime(1,time_step));		//density_vel_x
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormTime(2,time_step));		//density_vel_y
    add_matrix_form(new Euler_wf::EulerEquationsBilinearFormTime(3,time_step));		//energy
	...
	WeakForm<double>* Euler_wf::clone() const
    {
    Euler_wf* wf;
    wf = new Euler_wf(this->kappa,this->time_step, this->theta, this->rho_ext, this->v1_ext, this->v2_ext, this->energy_ext, this->prev_density, this->prev_density_vel_x, this->prev_density_vel_y, this->prev_energy, this->mirror_condition,4);
    ...
    template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerEquationsBilinearFormTime::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u, Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const 
    {
      return int_u_v<Real, Scalar>(n, wt, u, v)/time_step;
    }

    double Euler_wf::EulerEquationsBilinearFormTime::value(int n, double *wt, Func<double> *u_ext[], Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const 
    {
      return matrix_form<double, double>(n, wt, u_ext, u, v, e, ext);
    }

    Ord Euler_wf::EulerEquationsBilinearFormTime::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const 
    {
      return matrix_form<Ord, Ord>(n, wt, u_ext, u, v, e, ext);
    }

    MatrixFormVol<double>* Euler_wf::EulerEquationsBilinearFormTime::clone() const { return new Euler_wf::EulerEquationsBilinearFormTime(component_i, time_step); }

  template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerBoundaryBilinearForm_rho::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u, Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const  
{
	int bdry =0;
	double* ghost_state = new double[4];
  ...
  {		
		 ...
		 if(((static_cast<Euler_wf*>(wf))->mirror_condition==true)||(solid==false)){ 
				bdry =(static_cast<Euler_wf*>(wf))->riemann_invariants->get_bdry_info(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i],e->tx[i],e->ty[i], ext[4]->val[i], ext[5]->val[i], ext[6]->val[i],ext[7]->val[i], ghost_state, solid);
			...
			if(bdry!=1){
				...
				if(bdry!=2) 
				{
						(static_cast<Euler_wf*>(wf))->riemann_invariants->get_du_du(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i] ,e->tx[i], e->ty[i], ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3],bdry,j,dudu_j);//j-te Spalte
				}
			}

				if(j==0){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==1){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==2){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==3){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}
		      ...
		      result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				 ...
				 result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				result += wt[i]*v->val[i] *u->val[i]*0.5*(
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]  -A_n[0])* dudu_j[0] +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[1])*dudu_j[1]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[2])*dudu_j[2]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_0_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_0_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[3])*dudu_j[3] );
    ...
    double Euler_wf::EulerBoundaryBilinearForm_rho::value(int n, double *wt, Func<double> *u_ext[], Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const
    {
      return matrix_form<double, double>(n, wt, u_ext,u, v, e, ext);
    } 
    ...
    Ord Euler_wf::EulerBoundaryBilinearForm_rho::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const 
    {
      return Ord(20);
    }

    MatrixFormSurf<double>* Euler_wf::EulerBoundaryBilinearForm_rho::clone() const
    {
					return new Euler_wf::EulerBoundaryBilinearForm_rho(kappa,this->theta,  this->j);
    }

//vel_x
    template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerBoundaryBilinearForm_vel_x::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u, Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const  
{
	int bdry =0;
	double* ghost_state = new double[4];
  ...
  {			
	...
	if(bdry!=1){
		...
		if(bdry!=2) 
		{
				...
				if(j==0){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==1){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==2){
        result += wt[i] * u->val[i] *constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==3){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}
				if (bdry==2)
		      result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				 ...
				 result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				result += wt[i]*v->val[i] *u->val[i]*0.5*(
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]  -A_n[0])* dudu_j[0] +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[1])*dudu_j[1]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[2])*dudu_j[2]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_1_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_1_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[3])*dudu_j[3] );
				}
			}else{
						if(j==0){
    ...
    double Euler_wf::EulerBoundaryBilinearForm_vel_x::value(int n, double *wt, Func<double> *u_ext[],Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const 
    {
      return matrix_form<double, double>(n, wt, u_ext, u,v, e, ext);
    }

    Ord Euler_wf::EulerBoundaryBilinearForm_vel_x::ord(int n, double *wt, Func<Ord> *u_ext[],Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const 
    {
      return Ord(20);
    }

    MatrixFormSurf<double>* Euler_wf::EulerBoundaryBilinearForm_vel_x::clone() const { return new Euler_wf::EulerBoundaryBilinearForm_vel_x(kappa,this->theta, this->j); }

//vel_y
  template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerBoundaryBilinearForm_vel_y::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u,Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const 
    {
		int bdry =0;
		double* ghost_state = new double[4];
    ...
    {				
			...
			if(((static_cast<Euler_wf*>(wf))->mirror_condition==true)||(solid==false)){ 
				...
				if(j==0){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==1){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==2){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==3){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
           * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}
				 if (bdry==2)
		      result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				 ...
				 result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				result += wt[i]*v->val[i] *u->val[i]*0.5*(
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]  -A_n[0])* dudu_j[0] +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[1])*dudu_j[1]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[2])*dudu_j[2]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_2_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_2_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[3])*dudu_j[3] );
				}
    ...
    double Euler_wf::EulerBoundaryBilinearForm_vel_y::value(int n, double *wt, Func<double> *u_ext[],Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const 
    {
      return matrix_form<double, double>(n, wt, u_ext,u, v, e, ext);
    }

    Ord Euler_wf::EulerBoundaryBilinearForm_vel_y::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const 
    {
      return Ord(20);
    }

    MatrixFormSurf<double>* Euler_wf::EulerBoundaryBilinearForm_vel_y::clone() const { return new Euler_wf::EulerBoundaryBilinearForm_vel_y(kappa,this->theta, this->j); }


//energy
  template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerBoundaryBilinearForm_e::matrix_form(int n, double *wt, Func<Scalar> *u_ext[],Func<Real> *u, Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const 
 {
		int bdry =0;
		double* ghost_state = new double[4];
    ...
    {				
			...
			if(((static_cast<Euler_wf*>(wf))->mirror_condition==true)||(solid==false)){ 
				...
				if(j==0){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], ext[3]->val[i]) 
           * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_0<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], ext[3]->val[i]) 
           * e->ny[i]*v->val[i];
				}else if(j==1){
        result += wt[i] * u->val[i] *constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], ext[3]->val[i]) 
           * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_1<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}else if(j==2){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
           * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_2<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], ext[3]->val[i]) 
          * e->ny[i]*v->val[i];
				}else if(j==3){
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
           * e->nx[i]*v->val[i];
        result += wt[i] * u->val[i] * constant
        * (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_3<Scalar>(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i], Scalar(0)) 
          * e->ny[i]*v->val[i];
				}
				if (bdry==2)
		      result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				 ...
				 result += wt[i]*v->val[i] *u->val[i]*0.5*A_n[j];
				result += wt[i]*v->val[i] *u->val[i]*0.5*(
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_0<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]  -A_n[0])* dudu_j[0] +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_1<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[1])*dudu_j[1]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_2<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[2])*dudu_j[2]  +
				((static_cast<Euler_wf*>(wf))->euler_fluxes->A_1_3_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->nx[i]+
				 (static_cast<Euler_wf*>(wf))->euler_fluxes->A_2_3_3<Scalar>(ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3]) 
								  * e->ny[i]-A_n[3])*dudu_j[3] );
    ...
    double Euler_wf::EulerBoundaryBilinearForm_e::value(int n, double *wt, Func<double> *u_ext[],Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const 
    {
      return matrix_form<double, double>(n, wt, u_ext, u,v, e, ext);
    }

    Ord Euler_wf::EulerBoundaryBilinearForm_e::ord(int n, double *wt, Func<Ord> *u_ext[],Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const
    {
      return Ord(20);
    }

    MatrixFormSurf<double>* Euler_wf::EulerBoundaryBilinearForm_e::clone() const { return new Euler_wf::EulerBoundaryBilinearForm_e(kappa,this->theta, this->j); }
...
//---------------------K----------------
    ...
    template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerEquationsBilinearFormDensity::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u, Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const  
		 {
      Scalar result = Scalar(0);
      for (int i = 0;i < n;i++) 
    ...
    double Euler_wf::EulerEquationsBilinearFormDensity::value(int n, double *wt, Func<double> *u_ext[], Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const
    {
      return matrix_form<double, double>(n, wt, u_ext,u, v, e, ext);
    } 
    ...
    Ord Euler_wf::EulerEquationsBilinearFormDensity::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const 
    {
      return Ord(20);
    }

    MatrixFormVol<double>* Euler_wf::EulerEquationsBilinearFormDensity::clone() const
    {
    return new Euler_wf::EulerEquationsBilinearFormDensity(this->theta, this->j);
    }



    template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerEquationsBilinearFormDensityVelX::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u, Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const  
    {
      Scalar result = Scalar(0);
      for (int i = 0;i < n;i++) 
    ...
    double Euler_wf::EulerEquationsBilinearFormDensityVelX::value(int n, double *wt, Func<double> *u_ext[],Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const 
    {
      return matrix_form<double, double>(n, wt, u_ext, u,v, e, ext);
    }

    Ord Euler_wf::EulerEquationsBilinearFormDensityVelX::ord(int n, double *wt, Func<Ord> *u_ext[],Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const 
    {
      return Ord(20);
    }

    MatrixFormVol<double>* Euler_wf::EulerEquationsBilinearFormDensityVelX::clone() const { return new Euler_wf::EulerEquationsBilinearFormDensityVelX(kappa,this->theta, this->j); }



    template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerEquationsBilinearFormDensityVelY::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u,Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const 
    {
      Scalar result = Scalar(0);
      for (int i = 0;i < n;i++) 
    ...
    double Euler_wf::EulerEquationsBilinearFormDensityVelY::value(int n, double *wt, Func<double> *u_ext[],Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const 
    {
      return matrix_form<double, double>(n, wt, u_ext,u, v, e, ext);
    }

    Ord Euler_wf::EulerEquationsBilinearFormDensityVelY::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const 
    {
      return Ord(20);
    }

    MatrixFormVol<double>* Euler_wf::EulerEquationsBilinearFormDensityVelY::clone() const { return new Euler_wf::EulerEquationsBilinearFormDensityVelY(kappa,this->theta, this->j); }


  template<typename Real, typename Scalar>
    Scalar Euler_wf::EulerEquationsBilinearFormEnergy::matrix_form(int n, double *wt, Func<Scalar> *u_ext[],Func<Real> *u, Func<Real> *v, 
      Geom<Real> *e, Func<Scalar>  **ext) const 
    {
      Scalar result = Scalar(0);
      for (int i = 0;i < n;i++) 
    ...
    double Euler_wf::EulerEquationsBilinearFormEnergy::value(int n, double *wt, Func<double> *u_ext[],Func<double> *u, Func<double> *v, 
      Geom<double> *e, Func<double>  **ext) const 
    {
      return matrix_form<double, double>(n, wt, u_ext, u,v, e, ext);
    }

    Ord Euler_wf::EulerEquationsBilinearFormEnergy::ord(int n, double *wt, Func<Ord> *u_ext[],Func<Ord> *u, Func<Ord> *v, Geom<Ord> *e, 
      Func<Ord>  **ext) const
    {
      return Ord(20);
    }
    ...
    MatrixFormVol<double>* Euler_wf::EulerEquationsBilinearFormEnergy::clone() const { return new Euler_wf::EulerEquationsBilinearFormEnergy(kappa,this->theta, this->j); }
