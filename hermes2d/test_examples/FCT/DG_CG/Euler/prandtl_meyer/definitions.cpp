...
if((e->y[i]<=0.)&&(e->x[i]<2.)&&(e->x[i]>=-1.))	{	solid = true; bdry =0;}
		 ...
		 if(((static_cast<EulerKS*>(wf))->mirror_condition==true)||(solid==false)){ 
			//	bdry =(static_cast<EulerKS*>(wf))->riemann_invariants->get_bdry_info(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i],e->tx[i],e->ty[i], ext[4]->val[i], ext[5]->val[i], ext[6]->val[i],ext[7]->val[i], ghost_state, solid);
				...
				if((e->y[i]<=0.)&&(e->x[i]<2.)&&(e->x[i]>=-1.))	{	solid = true; bdry =0;}
				else if((e->y[i]==1.)||(e->x[i]==2.)){ bdry=1;}
...
A_n =  Boundary_helpers::calculate_A_n(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i] , ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3], kappa, 0); //0te-Zeile A_n
				if(bdry!=2) 
				{
...
if(bdry==1) constant =1.;
	...
	A_n=Boundary_helpers::calculate_A_n(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i] , ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3], kappa, 1);
		if(bdry!=2) 
		{
				(static_cast<EulerKS*>(wf))->riemann_invariants->get_du_du(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i] ,e->tx[i], e->ty[i], ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3],bdry,j,dudu_j);//j-te Spalte
		}
...
A_n= Boundary_helpers::calculate_A_n(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i] , ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3], kappa, 2); //2te-Zeile A_n
...
A_n = Boundary_helpers::calculate_A_n(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i] , ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3], kappa, 3);
					if(bdry!=2) 
					{
							(static_cast<EulerKS*>(wf))->riemann_invariants->get_du_du(ext[0]->val[i], ext[1]->val[i], ext[2]->val[i],ext[3]->val[i], e->nx[i],e->ny[i] ,e->tx[i], e->ty[i], ghost_state[0], ghost_state[1], ghost_state[2],ghost_state[3],bdry,j,dudu_j);//j-te Spalte
					}
...
//if((e->x[i]==-1.)||(e->x[i]==1)){
	...
	//determine free-stream values
			//	(static_cast<EulerKS*>(wf))->riemann_invariants->get_free_stream(rho, rho_v_x, rho_v_y, rho_energy, e->nx[i], e->ny[i], e->tx[i],e->ty[i],	new_variables, rho_ext, rho_v_x_ext, rho_v_y_ext, rho_energy_ext,bdry,solid);
