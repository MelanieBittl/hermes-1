...
double result =0.;	double diam = e->diam;

  for (int i = 0; i < n; i++) 
  {
double jump_u_x = exact->dx[i]-exact->dx_neighbor[i];
double jump_u_y = exact->dy[i]-exact->dy_neighbor[i];
double jump_v_x = v->dx[i];
double jump_v_y = v->dy[i];
		
result += wt[i]*EPS*(jump_u_x*jump_v_x+jump_u_y*jump_v_y)*diam*diam;
