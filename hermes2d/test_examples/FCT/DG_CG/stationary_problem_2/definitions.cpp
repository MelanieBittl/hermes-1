                                                ...
                                                Geom<double> *e, Func<double> **ext) const
{
	...
	double diam = e->diam;
		for (int i = 0; i < n; i++)
		{	
			//double grad_dot_n = static_cast<CustomWeakForm*>(wf)->calculate_a_dot_v(u->dx[i],u->dy[i], e->nx[i], e->ny[i]);
