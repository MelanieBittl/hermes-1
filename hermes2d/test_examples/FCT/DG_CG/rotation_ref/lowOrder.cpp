...
void Low_Order::assemble_Low_Order(CSCMatrix<double> * conv_matrix,CSCMatrix<double>* diffusion, CSCMatrix<double> * lumped_matrix,CSCMatrix<double> * surface_matrix,CSCMatrix<double> * dg_matrix)
{
	if(low_matrix!=NULL) low_matrix->free();	 		
	if(lowmat_rhs!=NULL) lowmat_rhs->free();

			lowmat_rhs->create(dg_matrix->get_size(),dg_matrix->get_nnz(), dg_matrix->get_Ap(), dg_matrix->get_Ai(),dg_matrix->get_Ax());
	lowmat_rhs->add_sparse_matrix(surface_matrix); 
	...
	//low_matrix->add_sparse_matrix(surface_matrix); 
			//(-theta)(K+D)
...
//lumped_matrix = M_L !!!!!
double* Low_Order::solve_Low_Order(CSCMatrix<double> * lumped_matrix, double* u_n,SimpleVector<double> * surf, double time_step  )
{
