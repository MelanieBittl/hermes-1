...
void High_Order::assemble_High_Order(CSCMatrix<double> * conv_matrix, CSCMatrix<double> * mass_matrix,CSCMatrix<double> * surface_matrix,CSCMatrix<double> * dg_matrix)
{
	if(high_matrix!=NULL) 
		high_matrix->free();	 		
	if(highmat_rhs!=NULL) 
		highmat_rhs->free();
			...
			high_matrix->create(dg_matrix->get_size(),dg_matrix->get_nnz(), dg_matrix->get_Ap(), dg_matrix->get_Ai(),dg_matrix->get_Ax());			
			high_matrix->add_sparse_matrix(surface_matrix); 
			high_matrix->add_sparse_matrix(conv_matrix); 
			highmat_rhs->create(high_matrix->get_size(),high_matrix->get_nnz(), high_matrix->get_Ap(), high_matrix->get_Ai(),high_matrix->get_Ax());
			
			high_matrix->multiply_with_Scalar(-theta);
			high_matrix->add_sparse_matrix(mass_matrix);  		

			//highmat_rhs->create(dg_matrix->get_size(),dg_matrix->get_nnz(), dg_matrix->get_Ap(), dg_matrix->get_Ai(),dg_matrix->get_Ax());
			//highmat_rhs->add_sparse_matrix(surface_matrix); 
...
double* High_Order::solve_High_Order(double* u_n,SimpleVector<double> * surf)
{
