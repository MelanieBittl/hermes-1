...
//FCT for projection 
void Flux_Correction::lumped_flux_limiter(CSCMatrix<double>* mass_matrix,CSCMatrix<double>* lumped_matrix, double* u_L, double* u_H, int* smooth_dof)
{	
  if(fct==NULL) 
    throw Exceptions::Exception("fct-list=NULL");
...
void Flux_Correction::project_FCT(MeshFunctionSharedPtr<double> sln,double* coeff_vec, double* coeff_vec_2,CSCMatrix<double>* mass_matrix,CSCMatrix<double>* lumped_matrix, OGProjection<double>* ogProjection,	Lumped_Projection* lumpedProjection, Regularity_Estimator* regEst)
{
  if(sln==NULL)
  ...
  lumped_flux_limiter(mass_matrix, lumped_matrix, coeff_vec, coeff_vec_2,smooth_dof); 
}
