...
ty_ghost = ty;
	...
	if((e->x[i]== x_min)&&(e->y[i]<=y_max)&&(e->y[i]>=y_min)) bdry = bdry_in;	
		 ...
		 if((mirror_condition==true)||(solid==false)){ 
				riemann_invariants->get_ghost_state(bdry,rho, rho_v_x, rho_v_y, rho_energy, nx_ghost,ny_ghost,tx_ghost,ty_ghost, rho_ext, rho_v_x_ext, rho_v_y_ext, rho_energy_ext, ghost_state);
...
boundary = bdry;
	...
	//determine free-stream values				
								riemann_invariants->get_ghost_state(bdry,rho, rho_v_x, rho_v_y, rho_energy, e->nx[i], e->ny[i], e->tx[i],e->ty[i],	 rho_ext, rho_v_x_ext, rho_v_y_ext, rho_energy_ext, new_variables);
