...
double QuantityCalculator::calc_pressure(double rho, double rho_v_x, double rho_v_y, double energy, double GAMMA)
{
  double to_return = (GAMMA - 1.0) * (energy - (rho_v_x*rho_v_x + rho_v_y*rho_v_y) / (2.0*rho));
  //if(std::abs(to_return) < 1E-12 || to_return < 0.0)
    //return 1E-12;
  if(to_return < 0.0)
    return 0.0;
...
double QuantityCalculator::calc_sound_speed(double rho, double rho_v_x, double rho_v_y, double energy, double GAMMA)
{
  ...
  // Calculates enthalpy.
double QuantityCalculator::enthalpy(double rho, double rho_v_x, double rho_v_y, double energy, double GAMMA){

double to_return= (energy+ QuantityCalculator::calc_pressure(rho, rho_v_x, rho_v_y,energy,GAMMA))/rho;
  //if(std::abs(to_return) < 1E-12 || to_return < 0.0)
    //return 1E-12;
 if(to_return < 0.0)
   return 0.0;
...
{
  for (int i = 0; i < n; i++)
    result[i] = QuantityCalculator::calc_pressure(values.at(0)[i], values.at(1)[i], values.at(2)[i], values.at(3)[i], gamma)/(values.at(0)[i]*R);
}
