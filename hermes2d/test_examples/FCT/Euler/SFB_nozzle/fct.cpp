...
void antidiffusiveFlux(CSCMatrix<double>* mass_matrix,CSCMatrix<double>* lumped_matrix,CSCMatrix<double>* diffusion,double* dt_u_L,	double* u_L, double* flux_scalar, double* P_plus, double* P_minus, double* Q_plus, double* Q_minus, double* R_plus, double* R_minus, int dof_rho, int dof_v_x, int dof_v_y, int dof_e )
{ 
...
int dof_v_y_start = dof_rho + dof_v_x ;
	...
	for(int i=0; i<ndof;i++){
						dt_u_L[i] *=1./(lumped_matrix->get(i,i)*time_step);	
		...
		//v_x
					...
					f_x= (f_rho_x- f_rho*v_x)/u_L[i];
						...
						f = lumped*((u_L[j+dof_rho]/u_L[j])-v_x); 
	...
	alpha = 1.0;
		for(int j = 0; j<dof_rho; j++){ //Spalten durchlaufen
				for(int indx = Ap_mass[j]; indx<Ap_mass[j+1];indx++){	
							int i = Ai_mass[indx];	
							if(((mass=(Ax_mass[indx]*time_step))!=0.)&&(j<dof_rho)){
										diff = diffusion->get(i,j);
							...
							v_x = (u_L[i+dof_rho]/u_L[i]);	
							v_y = (u_L[i+dof_v_y_start]/u_L[i]);
							f_p_ij=(GAMMA-1)*(f_rhoE+ f_rho*(v_x*v_x+v_y*v_y)*0.5-(v_x*f_rho_x+v_y*f_rho_y));
							v_x = (u_L[j+dof_rho]/u_L[j]);	
							v_y = (u_L[j+dof_v_y_start]/u_L[j]);
							f_p_ji = (GAMMA-1)*(-f_rhoE +f_rho_ji*(v_x*v_x+v_y*v_y)*0.5-(v_x*(-f_rho_x)+v_y*(-f_rho_y)));
