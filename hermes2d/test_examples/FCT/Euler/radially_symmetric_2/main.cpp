...
//--------------Weakforms------------
  EulerEquationsWeakForm_K*  wf_K_init = new EulerEquationsWeakForm_K(KAPPA, init_slns);
  EulerBoundary* wf_boundary_init = new EulerBoundary(KAPPA, Hermes::vector<MeshFunctionSharedPtr<double> >(init_rho, init_rho_v_x, init_rho_v_y, init_e,init_rho, init_rho_v_x, init_rho_v_y,  init_e),false );
  EulerEquationsWeakForm_Mass* wf_mass = new EulerEquationsWeakForm_Mass;
...
Hermes::vector<MeshFunctionSharedPtr<double> > (prev_rho, prev_rho_v_x, prev_rho_v_y, prev_e, init_rho, init_rho_v_x, init_rho_v_y,  init_e),false);
  EulerEquationsWeakForm_K*  wf_K_low = new EulerEquationsWeakForm_K(KAPPA, low_slns);
  EulerBoundary* wf_boundary_low = new EulerBoundary(KAPPA,Hermes::vector<MeshFunctionSharedPtr<double> > (low_rho, low_rho_v_x, low_rho_v_y, low_e, init_rho, init_rho_v_x, init_rho_v_y,  init_e ),false);


  //------------------- Initialize the FE problem.
  DiscreteProblem<double>* dp_mass= new DiscreteProblem<double>(wf_mass,spaces);
  DiscreteProblem<double>* dp_boundary_init= new DiscreteProblem<double>(wf_boundary_init,spaces);
  DiscreteProblem<double>* dp_K_init= new DiscreteProblem<double>(wf_K_init, spaces);
...
//---------------------Projection of the initial condition
    OGProjection<double>* ogProjection = new OGProjection<double>;
		Lumped_Projection* lumpedProjection= new Lumped_Projection;
...
delete wf_mass; delete dp_mass; delete ogProjection; delete lumpedProjection;
  	...
  	Hermes::Mixins::Loggable::Static::info("Time step %d, time %3.5f", ts, current_time); 	  
 	  if(ts!=1) {
		  ...
		  dp_K_init->assemble(lowmat_rhs);	
			delete dp_boundary_init; delete wf_boundary_init;
			delete dp_K_init;		delete 	wf_K_init;		
		}
 ...
 /* // Visualization.
    if((iteration - 1) % EVERY_NTH_STEP == 0) 
    {
      ...
      if(VTK_VISUALIZATION) 
      {
        ...
        char filename[40];
        sprintf(filename, "pressure_%i.vtk", iteration - 1);
        lin_pressure.save_solution_vtk(pressure, filename, "Pressure", true);
        Linearizer lin_v_r;
        sprintf(filename, "vel_r_%i.vtk", iteration - 1);
			lin_v_r.save_solution_vtk(vel_r, filename, "velocity_r",true);
        Linearizer lin_rho;
        sprintf(filename, "density_%i.vtk", iteration - 1);
...
sprintf(filename, "r-%i.vtk", ts );
			lin_r.save_solution_vtk(vel_r, filename, "radius", true);
...
mach->reinit();
        ...
        Linearizer lin_v_r;
			lin_v_r.save_solution_vtk(vel_r, "v_radius_end.vtk", "velocity_r",true);

			lin_rho.save_solution_vtk(prev_slns[0], "rho_end.vtk", "density", true);

			lin_m.save_solution_vtk(mach,"mach_end.vtk", "mach", true);
