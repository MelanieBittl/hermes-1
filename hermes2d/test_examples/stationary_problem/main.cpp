...
//View::wait(HERMES_WAIT_KEYPRESS);
 ...
 memset(coeff_vec_4, 0, ndof*sizeof(double));
	...
	CustomWeakForm wf_surf(u_prev_time,true, false);
...
*/
 ...
 MeshFunctionSharedPtr<double> sln_zero(new ZeroSolution<double>(mesh));
  ...
  errorCalculator_l2.calculate_errors(u_new, u_prev_time);
	...
	errorCalculator_sd.calculate_errors(u_new, sln_zero);
...
//fview.show(filter);
  ...
  OGProjection<double> ogProjection;	
        ogProjection.project_global(space, u_new, coeff_vec_3, HERMES_L2_NORM);	
		
  SpaceSharedPtr<double> space_test(new H1Space<double>(mesh,&bcs, P_INIT));	
...
for(int i =0; i<ndof;i++) 
{
	...
	//test+=rhs->get(i);
test += (coeff_vec_4[i]-surf_rhs->get(i));
