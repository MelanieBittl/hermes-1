...
/*
class EulerEquationsWeakFormSemiImplicitDoubleReflection : public WeakForm<double>
{
public:
  double kappa;
  bool fvm_only;
 ...
 Hermes::vector<std::string> solid_wall_markers;
  ...
  double exact_density_vel_x;
  double exact_density_vel_y;
  double exact_energy;
  ...
  EulerEquationsWeakFormSemiImplicitDoubleReflection(double kappa, 
    Hermes::vector<std::string> solid_wall_markers, Hermes::vector<std::string> inlet_markers, Hermes::vector<std::string> outlet_markers, 
    MeshFunctionSharedPtr<double> prev_density, MeshFunctionSharedPtr<double> prev_density_vel_x, MeshFunctionSharedPtr<double> prev_density_vel_y,  MeshFunctionSharedPtr<double> prev_energy, 
   double exact_density,double exact_density_vel_x,double exact_density_vel_y, double exact_energy, 
    bool fvm_only = false, int num_of_equations = 4) :
  WeakForm<double>(num_of_equations), 
    kappa(kappa), solid_wall_markers(solid_wall_markers), inlet_markers(inlet_markers), outlet_markers(outlet_markers), 
    prev_density(prev_density), prev_density_vel_x(prev_density_vel_x), prev_density_vel_y(prev_density_vel_y), prev_energy(prev_energy), 
    ...
    euler_fluxes(new EulerFluxes(kappa))
  {
    ...
    for(int form_i = 0; form_i < 4; form_i++)
    {
      ...
      add_vector_form_surf(new EulerEquationsVectorFormBdyFlux(form_i, exact_density, exact_density_vel_x, exact_density_vel_y, exact_energy, kappa, inlet_markers));
      add_vector_form_surf(new EulerEquationsVectorFormBdyFlux(form_i, exact_density, exact_density_vel_x, exact_density_vel_y, exact_energy, kappa, outlet_markers));
      ...
      for(int form_j = 0; form_j < 4; form_j++)
      {
        ...
        add_matrix_form_surf(new EulerEquationsMatrixFormBdyFlux(form_i, form_j, exact_density, exact_density_vel_x, exact_density_vel_y, exact_energy, kappa, inlet_markers, &this->cacheReadySurf, this->P_plus_cache_surf, this->P_minus_cache_surf));
        add_matrix_form_surf(new EulerEquationsMatrixFormBdyFlux(form_i, form_j, exact_density, exact_density_vel_x, exact_density_vel_y, exact_energy, kappa, outlet_markers, &this->cacheReadySurf, this->P_plus_cache_surf, this->P_minus_cache_surf));
  ...
  virtual ~EulerEquationsWeakFormSemiImplicitDoubleReflection()
  {
    delete this->euler_fluxes;
  ...
  WeakForm<double>* clone() const
  {
    EulerEquationsWeakFormSemiImplicitDoubleReflection* wf = new EulerEquationsWeakFormSemiImplicitDoubleReflection(this->kappa, this->solid_wall_markers,
      this->prescribed_markers, this->prev_density, this->prev_density_vel_x, this->prev_density_vel_y,  this->prev_energy, 
    this->exact_density, this->exact_density_vel_x, this->exact_density_vel_y,  this->exact_energy, 
    this->fvm_only, this->neq);

    wf->ext.clear();

    for(unsigned int i = 0; i < this->ext.size(); i++)
    {
      MeshFunctionSharedPtr<double> ext;
      if(dynamic_cast<Solution<double>*>(this->ext[i].get()))
      {
        if((dynamic_cast<Solution<double>*>(this->ext[i].get()))->get_type() == HERMES_SLN)
        {
          ext = new Solution<double>();
          ext->copy(this->ext[i].get());
          dynamic_cast<Solution<double>*>(ext.get())->set_type(HERMES_SLN);
        }
      }
      else
        ext = this->ext[i]->clone();
  ...
  class EulerEquationsMatrixFormBdyFlux : public MatrixFormSurf<double>
  {
  public:
    EulerEquationsMatrixFormBdyFlux(int i, int j,double exact_density,double exact_density_vel_x,double exact_density_vel_y, double exact_energy, double kappa, std::string marker, bool* cacheReady, double** P_plus_cache, double** P_minus_cache) 
      : MatrixFormSurf<double>(i, j), exact_density(exact_density), exact_density_vel_x(exact_density_vel_x), exact_density_vel_y(exact_density_vel_y), exact_energy(exact_energy), num_flux(new StegerWarmingNumericalFlux(kappa)), cacheReady(cacheReady), P_plus_cache(P_plus_cache), P_minus_cache(P_minus_cache)
    { 
      set_area(marker);
    }
    EulerEquationsMatrixFormBdyFlux(int i, int j,double exact_density,double exact_density_vel_x,double exact_density_vel_y, double exact_energy, double kappa, Hermes::vector<std::string> markers, bool* cacheReady, double** P_plus_cache, double** P_minus_cache) 
      : MatrixFormSurf<double>(i, j), exact_density(exact_density), exact_density_vel_x(exact_density_vel_x), exact_density_vel_y(exact_density_vel_y), exact_energy(exact_energy), num_flux(new StegerWarmingNumericalFlux(kappa)), cacheReady(cacheReady), P_plus_cache(P_plus_cache), P_minus_cache(P_minus_cache)
    { 
      set_areas(markers); 
    }

    ~EulerEquationsMatrixFormBdyFlux() 
    {
      delete num_flux;
    }
      ...
      Func<double> *v, Geom<double> *e, Func<double>* *ext) const 
    {
      ...
      if(!(*this->cacheReady))
      {
        for (int point_i = 0; point_i < n; point_i++) 
        {
          ...
          w_B[0] = exact_density;
          w_B[1] = exact_density_vel_x;
          w_B[2] = exact_density_vel_y;
          w_B[3] = exact_energy;
        ...
        *(const_cast<EulerEquationsMatrixFormBdyFlux*>(this))->cacheReady = true;
      }
    ...
    { 
      EulerEquationsMatrixFormBdyFlux* form = new EulerEquationsMatrixFormBdyFlux(this->i, this->j, this->exact_density, this->exact_density_vel_x, this->exact_density_vel_y, this->exact_energy, this->num_flux->kappa, this->areas, this->cacheReady, this->P_plus_cache, this->P_minus_cache);
      form->wf = this->wf;
      return form;
    }
  ...
  {
  public:
    EulerEquationsVectorFormBdyFlux(int i,double exact_density,double exact_density_vel_x,double exact_density_vel_y, double exact_energy, double kappa, std::string marker) 
      : VectorFormSurf<double>(i), exact_density(exact_density), exact_density_vel_x(exact_density_vel_x), exact_density_vel_y(exact_density_vel_y), exact_energy(exact_energy), num_flux(new StegerWarmingNumericalFlux(kappa))
    { 
      set_area(marker);
    }
    EulerEquationsVectorFormBdyFlux(int i,double exact_density,double exact_density_vel_x,double exact_density_vel_y, double exact_energy, double kappa, Hermes::vector<std::string> markers) 
      : VectorFormSurf<double>(i), exact_density(exact_density), exact_density_vel_x(exact_density_vel_x), exact_density_vel_y(exact_density_vel_y), exact_energy(exact_energy), num_flux(new StegerWarmingNumericalFlux(kappa))
    { 
      set_areas(markers); 
    }
    ...
    VectorFormSurf<double>* clone()  const
    { 
      EulerEquationsVectorFormBdyFlux* form = new EulerEquationsVectorFormBdyFlux(this->i, this->exact_density, this->exact_density_vel_x, this->exact_density_vel_y, this->exact_energy, this->num_flux->kappa, this->areas);
      form->wf = this->wf;
      return form;
    }
