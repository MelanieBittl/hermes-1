...
public:
    ...
    euler_fluxes(new EulerFluxes(kappa)), discreteIndicator(NULL)
  {
    ...
    for(int form_i = 0; form_i < 4; form_i++)
    {
      ...
      if(inlet_markers.size() > 0)
        add_vector_form_surf(new EulerEquationsVectorFormSemiImplicitInletOutlet(form_i, rho_ext_inflow, v1_ext_inflow, v2_ext_inflow, energy_ext_inflow, inlet_markers, kappa));

      if(outlet_markers.size() > 0)
        add_vector_form_surf(new EulerEquationsVectorFormSemiImplicitInletOutlet(form_i, rho_ext_outflow, v1_ext_outflow, v2_ext_outflow, energy_ext_outflow, outlet_markers, kappa));
      ...
      for(int form_j = 0; form_j < 4; form_j++)
      {
        ...
        if(inlet_markers.size() > 0)
        {
          EulerEquationsMatrixFormSemiImplicitInletOutlet* formSurf = new EulerEquationsMatrixFormSemiImplicitInletOutlet(form_i, form_j, rho_ext_inflow, v1_ext_inflow, v2_ext_inflow, energy_ext_inflow, inlet_markers, kappa, &this->cacheReadySurf, this->P_plus_cache_surf, this->P_minus_cache_surf);
          add_matrix_form_surf(formSurf);
        }

        if(outlet_markers.size() > 0)
        {
          EulerEquationsMatrixFormSemiImplicitInletOutlet* formSurf = new EulerEquationsMatrixFormSemiImplicitInletOutlet(form_i, form_j, rho_ext_outflow, v1_ext_outflow, v2_ext_outflow, energy_ext_outflow, outlet_markers, kappa, &this->cacheReadySurf, this->P_plus_cache_surf, this->P_minus_cache_surf);
          add_matrix_form_surf(formSurf);
        }
