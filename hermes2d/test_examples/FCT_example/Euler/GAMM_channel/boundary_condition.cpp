...
/*
double d_1 = std::fabs(n_x*rho_v_x/rho+n_y*rho_v_y/rho);
double d_2 = std::fabs(n_x*rho_v_x_new/rho_new+n_y*rho_v_y_new/rho_new);
if(d_1>d_2) return d_1*u[entry];
else return d_2*u[entry];*/
...
for(int i = 0;i<4; i++)
			result += A[entry][i]*u[i];*/


	for(int i =0;i<4;i++)
			for(int j=0;j<4;j++)
					result +=R[entry][j]*L[j][i]*u[i];

/*if(n_x==-1)
	if(result>1)
		printf("result=%f, u0=%f, u1=%f, u2=%f, u3=%f \n",result,u[0],u[1],u[2],u[3]);*/
