...
void smoothness_indicator(Space<double>* space,Solution<double>* sln,Solution<double>* R_h_1,Solution<double>* R_h_2, int* smooth_fct_in_elem, int* smooth_dx_in_elem, int* smooth_dy_in_elem,int* smooth_elem_patch, int* smooth_dof, AsmList<double>* al,bool proj){
...
double linear_approx_init(Element* e, double x_i, double y_i,double x_c, double y_c,
			...
			double y_c_ref = 0.; 
			double dx,dy;
			double u_h_x_c = sln->exact_function(x_c, y_c, dx, dy);
			double u_h_hat = u_h_x_c + R_h_1->get_ref_value(e, x_c_ref, y_c_ref, 0, 0)*(x_i-x_c)+R_h_2->get_ref_value(e, x_c_ref, y_c_ref, 0, 0)*(y_i-y_c);

return u_h_hat;
}

double linear_approx_dx_init(Element* e, double x_i, double y_i,double x_c, double y_c,ExactSolutionScalar<double>* sln, 
						Solution<double>* R_h_1,Solution<double>* R_h_2){
	...
	//Mittelpunkt des Referenzelements (Einheitsquadrat)	
		...
		double u_h_hat = dx + R_h_1->get_ref_value_transformed(e, x_c_ref, y_c_ref, 0, 1)*(x_i-x_c)
															 + R_h_2->get_ref_value_transformed(e, x_c_ref, y_c_ref, 0, 1)*(y_i-y_c);
return u_h_hat;

}

double linear_approx_dy_init(Element* e, double x_i, double y_i,double x_c, double y_c,ExactSolutionScalar<double>* sln,
				 Solution<double>* R_h_1,Solution<double>* R_h_2){
	...
	//Mittelpunkt des Referenzelements (Einheitsquadrat)	
		...
		double dx,dy; 
		double u_h_x_c = sln->exact_function(x_c, y_c, dx, dy);	
		double u_h_hat = dy + R_h_1->get_ref_value_transformed(e, x_c_ref, y_c_ref, 0, 2)*(x_i-x_c)
																+R_h_2->get_ref_value_transformed(e, x_c_ref, y_c_ref, 0, 2)*(y_i-y_c);
return u_h_hat;
...
//---------------Elemente mit Vertex(dof) bestimmen
	...
	for_all_active_elements(e, space->get_mesh()){
		...
		for (unsigned int iv = 0; iv < e->get_nvert(); iv++){ 		 
			...
			x_c/=e->get_nvert(); y_c/=e->get_nvert();
			u_c[e->id] = sln->exact_function(x_c, y_c, dx, dy);
			d_u_c_dx[e->id]= dx;
			d_u_c_dy[e->id]= dy;
			space->get_element_assembly_list(e, al);
			for (unsigned int iv = 0; iv < e->get_nvert(); iv++){ 
...
//------------------------------------------------
	...
	for_all_active_elements(e, space->get_mesh()){
		non_smooth = false; non_smooth_dx = false; non_smooth_dy = false;
		smooth_fct_in_elem[e->id]=1; //erstmal als smooth annehmen	
		smooth_dx_in_elem[e->id] =1; smooth_dy_in_elem[e->id] =1;
		space->get_element_assembly_list(e, al);		
			x_c = 0.; y_c = 0.;
		for (unsigned int iv = 0; iv < e->get_nvert(); iv++){ 		 
			 vn = e->vn[iv];
		...
		for (unsigned int iv = 0; iv < e->get_nvert(); iv++){ 			
			for(unsigned int j = 0; j < al->get_cnt(); j ++){			 
					if((al->get_idx()[j]==index[iv])&&(al->get_dof()[j]!=-1.0)){
							u_i = linear_approx_init(e, x[iv], y[iv],x_c, y_c,sln, R_h_1,R_h_2);	
							u_dx =	linear_approx_dx_init(e, x[iv], y[iv],x_c, y_c,sln, R_h_1,R_h_2);
							u_dy =	linear_approx_dy_init(e, x[iv], y[iv],x_c, y_c,sln, R_h_1,R_h_2);	
							u_min = u_c[e->id]; u_max =u_c[e->id];	
							u_min_dx= d_u_c_dx[e->id];	u_max_dx= d_u_c_dx[e->id];	
							u_min_dy= d_u_c_dy[e->id];	u_max_dy= d_u_c_dy[e->id];								
							for(elem_id=dof_elem_list[al->get_dof()[j]].begin();elem_id!=dof_elem_list[al->get_dof()[j]].end();elem_id++){	
