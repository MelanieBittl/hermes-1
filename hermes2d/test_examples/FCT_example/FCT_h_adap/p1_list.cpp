...
void p1_list_fast(Space<Scalar>* space, AsmList<Scalar>* dof_list,AsmList<Scalar>* dof_list_2, AsmList<Scalar>* al){
...
template<typename Scalar>
void p1_list(Space<Scalar>* space, AsmList<Scalar>* dof_list,AsmList<Scalar>* dof_list_2, AsmList<Scalar>* al, std::list<int>* list, std::list<int>* list_2 )
{
	int test =0;
	Element* e =NULL;
	Element* parent =NULL;
									...
									for(unsigned int j = 0; j < al->get_cnt(); j ++){			 
											if((al->get_idx()[j]==index)&&(al->get_dof()[j]!=-1.0)){ //dof=-1 =>dirichlet
												...
												if(more==false){
													...
													// if(more==false){ dof_list_2->add_triplet(1, al->get_dof()[j], 1.0);  test++;} //statt e->id, Anzahll an p1 setzen
													if(more==false) dof_list->add_triplet(e->id, al->get_dof()[j], 1.0);  //in dof_list einfuegen, da Kindelemente
