...
UMFPackMatrix<Scalar>* artificialDiffusion(AsmList<Scalar>* al,AsmList<Scalar>* al_2,UMFPackMatrix<Scalar>* conv_matrix)
{ //al=NULL => diffusion=0
		...
		for(unsigned int i = 0; i < al->get_cnt(); i ++){
		 for(unsigned int j = 0; j < al_2->get_cnt(); j ++){
			if(al->get_dof()[i]!=al_2->get_dof()[j]){		
						     a= -conv_matrix->get(al->get_dof()[i],al_2->get_dof()[j]);
						    	b= -conv_matrix->get(al_2->get_dof()[j],al->get_dof()[i]);     
						   if((a>=b)&&(a>0.0)){
						 diffusion->add(al_2->get_dof()[j],al->get_dof()[i],a);
						diffusion->add(al->get_dof()[i],al_2->get_dof()[j],a);	
						 diffusion->add(al_2->get_dof()[j],al_2->get_dof()[j],-a);
						diffusion->add(al->get_dof()[i],al->get_dof()[i],-a);	
						   }else if((b>a)&&(b>0.0)){
						 diffusion->add(al->get_dof()[i],al_2->get_dof()[j],b);
						diffusion->add(al_2->get_dof()[j],al->get_dof()[i],b);
					 	 diffusion->add(al_2->get_dof()[j],al_2->get_dof()[j],-b);
						diffusion->add(al->get_dof()[i],al->get_dof()[i],-b);
			   				}
				}
			}
