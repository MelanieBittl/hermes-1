...
// Time stepping loop:
	...
	Mass_DP* dp_lumped = new Mass_DP(&massmatrix, ref_space);
	UMFPackMatrix<double> * lumped_matrix = new UMFPackMatrix<double> ; 
...
//Adaptivity loop
	...
	{			int ref_ndof = ref_space->get_num_dofs();
			...
			if((ps==1))
					p1_list_fast(ref_space, dof_list,dof_list_2, al, P_plus,P_minus);
			else
					 p1_list(ref_space, dof_list,dof_list_2, al,&p1_elements,&neighbor, P_plus,P_minus);
	...
	for(unsigned int i = 0; i < dof_list->get_cnt(); i ++){
			 	for(unsigned int j = (i+1); j < dof_list->get_cnt(); j ++){
					if((dof_list->get_dof()[i]!=dof_list->get_dof()[j])&&((mass_matrix->get(dof_list->get_dof()[i],dof_list->get_dof()[j]))!=0.0)){		
						f = lumped_matrix->get(dof_list->get_dof()[i],dof_list->get_dof()[i])*(u_prev_time.get_pt_value(P_plus[dof_list->get_dof()[j]], P_minus[dof_list->get_dof()[j]])- u_prev_time.get_pt_value(P_plus[dof_list->get_dof()[i]], P_minus[dof_list->get_dof()[i]]))/time_step; 
						...
						f= lumped_matrix->get(dof_list->get_dof()[j],dof_list->get_dof()[j])*(u_prev_time.get_pt_value(P_plus[dof_list->get_dof()[i]], P_minus[dof_list->get_dof()[i]])- u_prev_time.get_pt_value(P_plus[dof_list->get_dof()[j]], P_minus[dof_list->get_dof()[j]]))/time_step; 
...
//info("Linear Solver lowOrd");
		...
		//smoothness_indicator(ref_space,&low_sln,&R_h_1,&R_h_2, smooth_fct_in_elem,smooth_dx_in_elem,smooth_dy_in_elem,smooth_elem,smooth_dof,al);
			//	info("assemble fluxes");	
			 antidiffusiveFlux(dof_list,dof_list_2,mass_matrix,lumped_matrix,conv_matrix,diffusion,vec_rhs, u_L, flux_double, 
									P_plus, P_minus, Q_plus, Q_minus,Q_plus_old, Q_minus_old, R_plus, R_minus );		
...
vec_rhs->zero(); 
    ...
    if(ps==2)u_prev_time.copy(&u_new);
		...
		if(ps==1){
			...
			changed = h_p_adap(ref_space, &u_new,&R_h_1,&R_h_2, adapting, &p1_elements,&neighbor,elements_to_refine,no_of_refinement_steps,h_min,h_max, ts,ps,smooth_elem);			
