...
template<typename Scalar>
//bool init_adapt(Space<Scalar>* space,ExactSolutionScalar<double>* sln,Solution<Scalar>* ref_sln, CustomWeakFormMassmatrix* massmatrix,HPAdapt* adapt, AsmList<Scalar>* dof_list,AsmList<Scalar>* dof_list_2, AsmList<Scalar>* al,  std::list<int>* list,std::list<int>* neighbor, double h_min, double h_max)
...
smoothness_indicator(space,ref_sln,R_h_1,R_h_2, smooth_fct_in_elem,smooth_dx_in_elem,smooth_dy_in_elem,smooth_elem,smooth_dof,al);
		...
		for_all_active_elements(e, space->get_mesh()){
				if(smooth_elem[e->id]==1){	//1=element is smooth
						refine = true; elements_to_refine[e->id] =2;no_of_refinement_steps[e->id]++;
				}
		}*/
...
info("h-Indicator");
		...
		Adapt<Scalar>* adaptivity_e_h = new Adapt<Scalar>(space, HERMES_L2_NORM);

		
		Scalar* zeros = new Scalar[space->get_num_dofs()]; for(int i =0; i<space->get_num_dofs(); i++) zeros[i] = 0.;
	...
	double tol_h = mean_value_h;
		...
		double tol_2 = mean_value_e_h-0.1*std_dev_e_h;
		if(tol_2<0.)tol_2 = 10*EPS;
		double tol_e_h = mean_value_e_h;
...
info("p-Indicator");
		...
		int* smooth_elem = new int[space->get_mesh()->get_max_element_id()];
		int* smooth_dof = new int[space->get_num_dofs()];
		smoothness_indicator(space,sln,R_h_1,R_h_2, smooth_fct_in_elem,smooth_dx_in_elem,smooth_dy_in_elem,smooth_elem,smooth_dof,al);

			double mean_value=0.;
			double std_dev = 0.0;	
			int counter_2 = 0;		
			for_all_active_elements(e, space->get_mesh()){
					if((elements_to_refine[e->id] != 1)&&(std::sqrt(adaptivity_e_h->get_element_error_squared(0, e->id))<tol_2)){
		...
		for_all_active_elements(e, space->get_mesh()){		
			if(elements_to_refine[e->id] != 1){
				if(std::sqrt(adaptivity_p->get_element_error_squared(0, e->id))>tol){	
							if(smooth_elem[e->id]==1){
							refine = true; elements_to_refine[e->id] =2;no_of_refinement_steps[e->id]++;
						}
				}									
...
info("h-Indicator");
			...
			if(tol_h<EPS) tol_h = EPS;	
			//double tol_h =EPS;
				
			double tol_h_2 = mean_value_h+std_dev_h;
			...
			//-----------p-Adapt
info("p-Indicator");
			Space<Scalar>* ref_space_p = construct_p1_space(space);
			int dof_p = ref_space_p->get_num_dofs();
			Scalar* coeff_2 = new Scalar[dof_p];
			...
			AsmList<Scalar> dof_list_p_2;
			p1_list_fast(ref_space_p, &dof_list_p,&dof_list_p_2, al);
			Scalar* coeff_4 = new Scalar[dof_p];
			Scalar* P_plus_p = new Scalar[dof_p]; Scalar* P_minus_p = new Scalar[dof_p];
			Scalar* Q_plus_p = new Scalar[dof_p]; Scalar* Q_minus_p = new Scalar[dof_p];	
			Scalar* R_plus_p = new Scalar[dof_p]; Scalar* R_minus_p = new Scalar[dof_p];	
			DiscreteProblem<Scalar>* dp_mass_p = new DiscreteProblem<Scalar>(massmatrix, ref_space_p);
			UMFPackMatrix<Scalar>* mass_matrix_p = new UMFPackMatrix<Scalar>; 
			dp_mass_p->assemble(mass_matrix_p); 	
			UMFPackMatrix<Scalar>* lumped_matrix_p = massLumping(&dof_list_p,&dof_list_p_2,mass_matrix_p);
			lumped_matrix_p->multiply_with_Scalar(time_step);  // M_L
			mass_matrix_p->multiply_with_Scalar(time_step);  // massmatrix = M_C
			Lumped_Projection::project_lumped(ref_space_p, sln, coeff_2, matrix_solver, lumped_matrix_p);
			OGProjection<Scalar>::project_global(ref_space_p,sln, coeff_4, matrix_solver, HERMES_L2_NORM);
			lumped_flux_limiter(&dof_list_p,&dof_list_p_2,mass_matrix_p, lumped_matrix_p, coeff_2, coeff_4,
									P_plus_p, P_minus_p, Q_plus_p, Q_minus_p, R_plus_p, R_minus_p );
			Solution<Scalar>::vector_to_solution(coeff_2, ref_space_p, u_2h_2p);

			//OGProjection::project_global(ref_space_p,sln, coeff_2, matrix_solver, HERMES_L2_NORM);
			//Solution::vector_to_solution(coeff_2, ref_space_p, u_2h_2p);

			Adapt<Scalar>* adaptivity_p = new Adapt<Scalar>(space, HERMES_L2_NORM);	
			err_est = adaptivity_p->calc_err_est(sln,u_2h_2p,true,HERMES_TOTAL_ERROR_ABS|HERMES_ELEMENT_ERROR_ABS) ;

			double mean_value=0.;
			double std_dev = 0.0;	
			int counter_2 = 0;		
			for_all_active_elements(e, space->get_mesh()){
				//if(elements_to_refine[e->id] != 4){
					mean_value += std::sqrt(adaptivity_p->get_element_error_squared(0, e->id));
					std_dev += adaptivity_p->get_element_error_squared(0, e->id);
					counter_2++;
				//}		
		...
		//	if(tol>EPS) tol = EPS;
			...
			for_all_active_elements(e, space->get_mesh()){		
						if(std::sqrt(adaptivity_p->get_element_error_squared(0, e->id))<tol){			
							if(elements_to_refine[e->id] != 4){
										 elements_to_refine[e->id] =3; 
							}/*else{ 
										elements_to_refine[e->id] =6; no_of_refinement_steps[e->id]--; 
							}*/
			...
			delete [] coeff_4;
			delete dp_mass_p;
			delete lumped_matrix_p; 
			delete mass_matrix_p; 
						delete adaptivity_p;
			delete [] coeff_2;
			delete ref_space_p->get_mesh();
			delete ref_space_p;
