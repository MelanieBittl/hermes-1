...
if(fct!=NULL)if(fct[i]==true) alpha =1.;
...
//FCT for lumped projection
template<typename Scalar>
void lumped_flux_limiter(UMFPackMatrix<Scalar>* mass_matrix,UMFPackMatrix<Scalar>* lumped_matrix, Scalar* u_L, Scalar* u_H, Scalar* P_plus, Scalar* P_minus, Scalar* Q_plus, Scalar* Q_minus,Scalar* R_plus, Scalar* R_minus, int dof_rho, int dof_v_x, int dof_v_y, int dof_e, double* alpha_node)
{	
	int ndof = mass_matrix->get_size();
	UMFPackVector<Scalar>* vec_rhs = new UMFPackVector<Scalar>(ndof);	
	vec_rhs->zero(); 
							...
							if(((mass=Ax_mass[indx])!=0.)&&(j<i)){
										...
										if(f>0.){					
											...
											if(R_minus[i]>R_plus[j]) alpha = R_plus[j];
												...
												rhs[j] -= alpha*f;
											i_k = i; j_k=j;
											while(i_k>=dof_rho) i_k -=dof_rho;
											while(j_k>=dof_rho) j_k -=dof_rho;
											if(alpha_node[i_k]<alpha) alpha_node[i_k] = alpha;
											if(alpha_node[j_k]<alpha) alpha_node[j_k] = alpha;
										}											
							}
				}			
		}
