...
void antidiffusiveFlux(EpetraMatrix<double>* mass_matrix,EpetraMatrix<double>* lumped_matrix,EpetraMatrix<double>* diffusion,EpetraMatrix<double>* matrix_L_low,	double* u_L, double* flux_scalar, double* P_plus, double* P_minus, double* Q_plus, double* Q_minus, double* R_plus, double* R_minus, int dof_rho, int dof_v_x, int dof_v_y, int dof_e,bool* fct = NULL, int* smooth_dof=NULL  )
{ 
 ...
 int n_entries;
	double* vals = new double[ndof];
 int* idxs = new  int[ndof];
	int j;
	...
	for(int i=0; i<ndof;i++){
 	 ...
 	 for(int i = 0; i<dof_rho; i++) {
 			...
 			mass_matrix->extract_row_copy(i, dof_rho, n_entries, vals, idxs);
				for(int indx =0;indx<n_entries;indx++)
						if((mass=vals[indx])!=0) {
								j = idxs[indx];
								if(fct!=NULL)if(fct[j]== false) continue;		
								if(j!=i){
	/*for(unsigned int i = 0; i<dof_rho; i++){
				for(int j=0; j< dof_rho; j++){		
						if((mass=mass_matrix->get(i,j))!=0.){		*/	
							diff=diffusion->get(i,j);
							lumped= lumped_matrix->get(i,i);
...
void lumped_flux_limiter(EpetraMatrix<Scalar>* mass_matrix,EpetraMatrix<Scalar>* lumped_matrix, Scalar* u_L, Scalar* u_H, Scalar* P_plus, Scalar* P_minus, Scalar* Q_plus, Scalar* Q_minus,Scalar* R_plus, Scalar* R_minus, bool* fct = NULL, int* smooth_dof=NULL  )
{	
 int ndof = mass_matrix->get_matrix_size();
	EpetraVector<Scalar>* vec_rhs = new EpetraVector<Scalar>;	
	vec_rhs->alloc(ndof);
...
double* vals = new double[ndof];
 int* idxs = new int[ndof];
	...
	for( int i = 0; i<ndof; i++){
						if(fct!=NULL)if(fct[i]== false) continue;
			mass_matrix->extract_row_copy(i, ndof, n_entries, vals, idxs);
				for(int indx =0;indx<n_entries;indx++){
						if((mass=vals[indx])!=0) {
								j = idxs[indx];
								if(fct!=NULL)if(fct[j]== false) continue;
	...
	double* sol =NULL;
	AztecOOSolver<double>* lowOrd = new AztecOOSolver<double>(lumped_matrix,vec_rhs);
