...
void calc_elem_error(Space<Scalar>* space, Solution<Scalar>* sln,Solution<Scalar>* R_h_1,Solution<Scalar>* R_h_2, double h_min, double h_max, 	int* elements_to_refine,	int* no_of_refinement_steps,double* elem_error)
{		
...
//-------------- aus reg_estimator------------------------------
	...
	EpetraVector<double> * rhs_1 = new EpetraVector<double>;
	EpetraVector<double> * rhs_2 = new EpetraVector<double>;
		rhs_1->alloc(ndof);
		rhs_2->alloc(ndof);
	...
	if((elements_to_refine[e->id]==1)&&(visited[e->id]==false)){ 
			...
			if(elem_start_1->vn[0]->y == elem_start_1->vn[1]->y){ 
						...
						if(elem_start_1->vn[2]->y == elem_start_1->vn[3]->y)
							 e_2 =2;
						else error("calc_elem_error:falsche y-Koord");
			}						
...
if( elem_start_2->get_neighbor(e_2)!=NULL) 
	...
	for_all_active_elements(e, space->get_mesh()) 
	{
		...
		{ 
				...
				}else error("calc_elem_error:keine zweit knoten selbe y-Koord.");
