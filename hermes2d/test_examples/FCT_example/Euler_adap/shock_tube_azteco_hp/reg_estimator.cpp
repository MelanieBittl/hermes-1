...
if(new_space == NULL) throw Hermes::Exceptions::Exception("smoothness_indicator: set_space");
		...
		rhs_1 = new EpetraVector<double>;
				rhs_1->alloc(ndof);
		rhs_2 = new EpetraVector<double>;
		rhs_2->alloc(ndof);
}

int* Regularity_Estimator::get_smooth_elems(Space<double>* new_space,double* coeff_vec,EpetraMatrix<double> * mass_matrix)
{
	if(new_space == NULL) throw Hermes::Exceptions::Exception("smoothness_indicator: space =NULL");
		...
		if(space!=new_space) set_space(new_space);
				if(grad_1!=NULL) delete grad_1;
		if(grad_2!=NULL) delete grad_2;
...
int* Regularity_Estimator::get_smooth_dofs(Space<double>* new_space,double* coeff_vec,EpetraMatrix<double> * mass_matrix)
{
	if(new_space == NULL) throw Hermes::Exceptions::Exception("smoothness_indicator: space =NULL");
		if(coeff_vec==NULL) throw Hermes::Exceptions::Exception("smoothness_indicator: coeff_vec=NULL");
	//if(space!=new_space) set_space(new_space);
...
int* Regularity_Estimator::get_smooth_elems(Space<double>* new_space, Solution<double>* new_sln,EpetraMatrix<double> * mass_matrix)
{
	if(new_space == NULL) throw Hermes::Exceptions::Exception("smoothness_indicator: space =NULL");
		if(space!=new_space) set_space(new_space);
		sln = new_sln;
...
int* Regularity_Estimator::get_smooth_dofs(Space<double>* new_space, Solution<double>* new_sln,EpetraMatrix<double> * mass_matrix)
{
	if(new_space == NULL) throw Hermes::Exceptions::Exception("smoothness_indicator: space =NULL");
		if(space!=new_space) set_space(new_space);
	sln = new_sln;	
...
double Regularity_Estimator::linear_approx_dx(Element* e, double x_i, double y_i,double x_c, double y_c,Solution<double>* sln){
	...
	//center of reference element	
		...
		double u_h_hat_1 = d_u_h_x_c + R_h_1->get_ref_value_transformed(e, x_c_ref, y_c_ref, 0, 1)*(x_i-x_c)
...
double Regularity_Estimator::linear_approx_dy(Element* e, double x_i, double y_i,double x_c, double y_c,Solution<double>* sln){
	...
	//center of reference element	
		...
		double u_h_hat_2 = d_u_h_x_c + R_h_2->get_ref_value_transformed(e, x_c_ref, y_c_ref, 0, 1)*(x_i-x_c)
...
void Regularity_Estimator::smoothness_indicator(EpetraMatrix<double> * mass_matrix)
{
	int ndof = space->get_num_dofs(); 
	...
	if(mass_matrix ==NULL) 
	{
		 mass_matrix = new EpetraMatrix<double> ; 	
	...
	AztecOOSolver<double> * solver_1 = new AztecOOSolver<double> (mass_matrix,rhs_1);
	AztecOOSolver<double> * solver_2 = new AztecOOSolver<double> (mass_matrix,rhs_2);	
	...
	std::list<int> dof_elem_list[ndof];	
		...
		Element* e =NULL; 
		int* index = new int[4];
		for(int i =0;i<4;i++)  index[i] =  space->get_shapeset()->get_vertex_index(i);
...
//R_H^1
 GradientReconstruction_1::GradientReconstruction_1( Solution<double>* sln) : WeakForm<double>(1) {
    add_matrix_form(new GradientReconstructionMatForm(0, 0));
    ...
    Scalar GradientReconstructionMatForm ::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u, 
...
double GradientReconstructionMatForm ::value(int n, double *wt, Func<double> *u_ext[], Func<double> *u, 
                 Func<double> *v, Geom<double> *e, ExtData<double> *ext) const {
      return matrix_form<double, double>(n, wt, u_ext, u, v, e, ext);
    };
Ord GradientReconstructionMatForm ::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v, 
            Geom<Ord> *e, ExtData<Ord> *ext) const {
      return matrix_form<Ord, Ord>(n, wt, u_ext, u, v, e, ext);
    };

   MatrixFormVol<double>* GradientReconstructionMatForm::clone()
{
  return new GradientReconstructionMatForm(*this);
}
