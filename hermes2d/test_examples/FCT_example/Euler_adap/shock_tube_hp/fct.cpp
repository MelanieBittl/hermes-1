...
void antidiffusiveFlux(UMFPackMatrix<double>* mass_matrix,UMFPackMatrix<double>* lumped_matrix,UMFPackMatrix<double>* diffusion,UMFPackMatrix<double>* matrix_L_low,	double* u_L, double* flux_scalar, double* P_plus, double* P_minus, double* Q_plus, double* Q_minus, double* R_plus, double* R_minus, int dof_rho, int dof_v_x, int dof_v_y, int dof_e,bool* fct = NULL, int* smooth_dof=NULL  )
{ 
...
{ 
	...
	for(int j = 0; j<dof_rho; j++)
	{ //Spalten durchlaufen
			if(fct!=NULL)if(fct[j]== false) continue;
		for(int indx = Ap_mass[j]; indx<Ap_mass[j+1];indx++)
		{	
			int i = Ai_mass[indx];
			if(fct!=NULL)if(fct[i]== false) continue;	
			if(((mass=Ax_mass[indx])!=0.))
			{
				diff=diffusion->get(i,j);
				lumped= lumped_matrix->get_Ax()[i];
...
/*		//v_x
	...
	{			if(fct!=NULL)if(fct[i]== false) continue;
		if((i<dof_rho)||(i>dof_rhoE_start))
		{
			...
			if(smooth_dof!=NULL)if(smooth_dof[i]==true) { smooth_dof[i]= 1.0;smooth_dof[i]= 1.0;}
		}
	}	
...
void lumped_flux_limiter(UMFPackMatrix<Scalar>* mass_matrix,UMFPackMatrix<Scalar>* lumped_matrix, Scalar* u_L, Scalar* u_H, Scalar* P_plus, Scalar* P_minus, Scalar* Q_plus, Scalar* Q_minus,Scalar* R_plus, Scalar* R_minus, int dof_rho, int dof_v_x, int dof_v_y, int dof_e,bool* fct = NULL,int* smooth_dof=NULL  )
{	
