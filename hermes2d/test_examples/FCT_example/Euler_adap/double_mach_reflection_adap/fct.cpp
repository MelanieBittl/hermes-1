...
{ 
	...
	{ //Spalten durchlaufen
		...
		//v_x
				...
				f = lumped*((u_L[j+dof_rho]/u_L[j])-v_x_i)/time_step; 
		...
		//v_y
				...
				f = lumped*((u_L[j+dof_v_y_start]/u_L[j])-v_y_i)/time_step; 
	...
	//density- rho
		...
		//velocity y
				...
				double alpha_v = (alpha_v_x*Hermes::sqr(v_x_i)+ alpha_v_y*Hermes::sqr(v_y_i))/Hermes::sqr(v_ij);
...
{ 
	...
	{ //Spalten durchlaufen
		...
		{	
			...
			if(f_ij>=0.){//R_plus[i]					
				if(f_ji>=0.){//R_plus[j]	
						if(R_plus[i+dof_rhoE_start]>R_plus[j+dof_rhoE_start]) alpha_rho = R_plus[j+dof_rhoE_start];
						else alpha_E = R_plus[i+dof_rhoE_start];
				}else{//R_minus[j]
					if(R_plus[i+dof_rhoE_start]>R_minus[j+dof_rhoE_start]) alpha_rho = R_minus[j+dof_rhoE_start];
					else alpha_E = R_plus[i+dof_rhoE_start];
				}	
		}else{//R_minus[i]
				if(f_ji>=0.){//R_plus[j]	
						if(R_minus[i+dof_rhoE_start]>R_plus[j+dof_rhoE_start]) alpha_rho = R_plus[j+dof_rhoE_start];
						else alpha_E = R_minus[i+dof_rhoE_start];
				}else{//R_minus[j]
					if(R_minus[i+dof_rhoE_start]>R_minus[j+dof_rhoE_start]) alpha_rho = R_minus[j+dof_rhoE_start];
					else alpha_E = R_minus[i+dof_rhoE_start];
				}	
		}
