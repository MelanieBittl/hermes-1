...
bool RiemannInvariants::solid_wall(double rho, double rho_v_x, double rho_v_y, double n_x, double n_y){
	if(fabs(rho_v_x*n_x/rho + rho_v_y*n_y/rho)==0.) 
				return true;
	else false;
...
int RiemannInvariants::get_bdry_info(double rho, double rho_v_x, double rho_v_y, double rho_energy, double n_x, double n_y, double t_x, double t_y,
	...
	if( solid==true){
		...
		ghost_state[2] = rho_v_y- 2*n_y*( rho_v_x*n_x+ rho_v_y*n_y); 
			return 0;
	/*}else if (RiemannInvariants::solid_wall(rho,rho_v_x, rho_v_y, n_x, n_y)==true){
		ghost_state[0] = rho;
		ghost_state[3] = rho_energy;
		ghost_state[1] = rho_v_x - 2*n_x*( rho_v_x*n_x+ rho_v_y*n_y); 	
		ghost_state[2] = rho_v_y- 2*n_y*( rho_v_x*n_x+ rho_v_y*n_y); 
		//printf("RiemannInvariants::get_bdry_info   :  solid wall detected");
		return 0;*/
		...
		return 4;	
				...
				if((lambda_1<0)&&(lambda_2_3>=0)&&(lambda_4<0)) error("case1");
				else	if((lambda_1>=0)&&(lambda_2_3<0)&&(lambda_4>=0)) error("case2");
				else	if((lambda_1>=0)&&(lambda_2_3>=0)&&(lambda_4<0)) error("case3");
				else error("get_bdry_info: eigenvalues = %f, %f, %f", lambda_1, lambda_2_3, lambda_4);
...
void RiemannInvariants::get_du_du(double rho, double rho_v_x, double rho_v_y, double rho_energy, double n_x, double n_y,double t_x, double t_y,
	...
	if(bdry ==3){//subsonic inlet
			...
			if(entry_j==0){
					...
					dw4_du = kappa/(c*rho);
			}else error("RiemannInvariants::get_du_du: no entry_j!");
	...
	}else  error("RiemannInvariants::get_du_du: no subsonic stream!, bdry = %i", bdry);
