...
//------Matrix & Vektorform for higher Order solution----------------
 ...
 template<typename Real, typename Scalar>
    Scalar ConvectionMatForm::matrix_form(int n, double *wt, Func<Scalar> *u_ext[], Func<Real> *u, 
         ...
         Scalar result = Scalar(0); 
				for (int i = 0; i < n; i++) 
    			result += wt[i] * (u->val[i] * v->val[i] / tau + 0.5*v->val[i] *(u->dx[i] * (e->y[i]) + u->dy[i] * (-e->x[i]) ));
  return result;
    }
  ...
  Func<Scalar>* u_prev_time = ext->fn[0];
  for (int i = 0; i < n; i++) 
    result += wt[i] * ((u_prev_time->val[i] * v->val[i]) / tau - 0.5*v->val[i] *(u_prev_time->dx[i] * (e->y[i]) + u_prev_time->dy[i] * (-e->x[i]) ));
